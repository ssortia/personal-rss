...
- **Настройка предпочтений** — описание интересов в свободной форме; LLM ориентируется на них при отборе; поддерживаются глобальные настройки и переопределение для каждого источника отдельно
- **Персональный RSS** — стандартный RSS 2.0-фид с отобранными новостями
- **Telegram-уведомления** — автоматическая доставка релевантных статей в личный чат раз в минуту; Telegram-посты пересылаются напрямую, RSS-статьи форматируются как HTML-сообщения
- **OAuth-вход** — регистрация и вход через Google, GitHub, Яндекс; при совпадении email провайдер автоматически привязывается к существующему аккаунту
